...
export class AccountEndpoint extends EndpointBase {
  ...
  get promoCode() { return this.configurations.baseUrl + '/api/promoCode'; }
  get generate() { return this.configurations.baseUrl + '/api/promoCode/generate'; }
  get list() { return this.configurations.baseUrl + '/api/promoCode/list'; }
  ...
  getListEndpoint<T>(): Observable<T> {
    const endpointUrl = this.list

    return this.http.get<T>(endpointUrl, this.requestHeaders).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.getListEndpoint());
      }));
  }

  addPromoCodeEndpoint<T>(userObject: any): Observable<T> {

    return this.http.post<T>(this.generate, JSON.stringify(userObject), this.requestHeaders).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.addPromoCodeEndpoint(userObject));
      }));
  }

  updatePromoCodeEndpoint<T>(id?: string): Observable<T> {
    const endpointUrl =  `${this.promoCode}/${id}`;

    return this.http.put<T>(endpointUrl, null, this.requestHeaders).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.updatePromoCodeEndpoint(id));
      }));
  }
